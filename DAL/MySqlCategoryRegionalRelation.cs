...
{
	/// <summary>
	/// 数据访问类:CategoryRegionalRelation
	/// </summary>
	public partial class MySqlCategoryRegionalRelation
	{
        public MySqlCategoryRegionalRelation()
		{}
		#region  BasicMethod
		...
		public bool Exists(long AutoID)
		{
			...
			strSql.Append("select count(1) from CategoryRegionalRelation");
		...
		/// 增加一条数据
		/// </summary>
		public long Add(Model.CategoryRegionalRelation model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.CategoryRegionalRelation model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update CategoryRegionalRelation set ");
			strSql.Append("CategotyID=@CategotyID,");
		...
		public bool Delete(long AutoID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from CategoryRegionalRelation ");
			strSql.Append(" where AutoID=@AutoID");
			MySqlParameter[] parameters = {
					new MySqlParameter("@AutoID", MySqlDbType.Int64)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CategoryRegionalRelation GetModel(long AutoID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select AutoID,CategotyID,RegionalID from CategoryRegionalRelation");
			...
			Model.CategoryRegionalRelation model=new Model.CategoryRegionalRelation();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CategoryRegionalRelation DataRowToModel(DataRow row)
		{
			Model.CategoryRegionalRelation model=new Model.CategoryRegionalRelation();
			if (row != null)
			{
				...
				if(row["CategotyID"]!=null && row["CategotyID"].ToString()!="")
				{
					model.CategotyID=long.Parse(row["CategotyID"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select AutoID,CategotyID,RegionalID ");
			strSql.Append(" FROM CategoryRegionalRelation ");
			if(strWhere.Trim()!="")
			{
        ...
        private DataSet GetList(int Top, string strWhere, string filedOrder)
		{
			...
			strSql.Append(" AutoID,CategotyID,RegionalID ");
			strSql.Append(" FROM CategoryRegionalRelation ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        public DataSet GetList(int key, string value, int Top, int filedOrder, bool desc)
        {
            ...
            switch (key)
            {
                case 0: strWhere = "AutoID = @AutoID";
                    ...
                    break;
                case 1: strWhere = "CategotyID = @CategotyID";
                    parameter = new MySqlParameter("@CategotyID", MySqlDbType.Int64);
                    parameter.Value = value;
                    break;
            ...
            switch (filedOrder)
            {
                ...
                case 1: strOrder = "CategotyID"; break;
        ...
        public DataSet GetListByPage(int key, string value, int filedOrder, bool desc, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from CategoryRegionalRelation T ");
        ...
        /// <returns></returns>
        public DataSet getCategoryList(string regionalIds, long parentCategoryId)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT A.CATEGOTYID FROM CATEGORYREGIONALRELATION A,CATEGORY B WHERE 1=1 ");
            strSql.Append("AND A.CATEGOTYID=B.AUTOID ");
            strSql.Append("AND A.REGIONALID in ('"+regionalIds+"') ");
            strSql.Append("AND B.PARENTCATEGORYID=@parentCategoryId ");
            strSql.Append("AND B.ISVALID=1 ");
            strSql.Append("Group by A.CATEGOTYID Order by B.Orders");
            MySqlParameter[] parameters = { 
