...
namespace DAL
{
    public partial class MySqlNoticeRegionalRelation
    {
        public MySqlNoticeRegionalRelation()
		{}
		#region  BasicMethod
		...
		public bool Exists(long AutoID)
		{
            ...
            strSql.Append("select count(1) from NoticeRegionalRelation");
		...
		/// 增加一条数据
		/// </summary>
        public bool Add(Model.NoticeRegionalRelation model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(Model.NoticeRegionalRelation model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		public bool Delete(long AutoID)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from NoticeRegionalRelation ");
			strSql.Append(" where AutoID=@AutoID ");
			MySqlParameter[] parameters = {
					new MySqlParameter("@AutoID", MySqlDbType.Int64)			};
			parameters[0].Value = AutoID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.NoticeRegionalRelation GetModel(long AutoID)
		{
            ...
            Model.NoticeRegionalRelation model = new Model.NoticeRegionalRelation();
		...
		/// 得到一个对象实体
		/// </summary>
        public Model.NoticeRegionalRelation DataRowToModel(DataRow row)
		{
            Model.NoticeRegionalRelation model = new Model.NoticeRegionalRelation();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select AutoID,NoticeID,RegionalID ");
            strSql.Append(" from NoticeRegionalRelation ");
			if(strWhere.Trim()!="")
			{
        ...
        public DataSet GetList(int key, string value, int Top, int filedOrder, bool desc)
        {
            ...
            switch (key)
            {
                case 0: strWhere = "AutoID = @AutoID";
                    ...
                    break;
                case 1: strWhere = "NoticeID = @NoticeID";
                    parameter = new MySqlParameter("@NoticeID", MySqlDbType.Int64);
                    parameter.Value = value;
                    break;
            ...
            switch (filedOrder)
            {
                ...
                case 1: strOrder = "NoticeID"; break;
        ...
        public DataSet GetListByPage(int key, string value, int filedOrder, bool desc, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from NoticeRegionalRelation T ");
