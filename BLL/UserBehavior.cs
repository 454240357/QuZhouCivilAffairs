...
namespace BLL
{
    ...
    {
      private readonly DAL.MySqlUserBehavior dal=new DAL.MySqlUserBehavior();
      public UserBehavior()
      {}

      public long Add(Model.UserBehavior model)
      {
      ...
      /// <param name="endT">时间段的结束时间。不需要设置结束时间可将参数设为DateTime.MaxValue</param> 
      /// <returns></returns>
      public List<Model.UserBehavior> GetModelList(string SimCardNO, string IpAddr, string Remarks, string Behavior, DateTime beginT, DateTime endT)
      {
      ...
      /// 将DateTable转成实体类集合
      /// </summary>
      public List<Model.UserBehavior> DataTableToList(DataTable dt)
      {
          List<Model.UserBehavior> modelList = new List<Model.UserBehavior>();
          int rowsCount = dt.Rows.Count;
          if (rowsCount > 0)
          {
              Model.UserBehavior model;
