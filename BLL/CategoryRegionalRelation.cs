...
namespace BLL
{
	...
	{
        private readonly DAL.MySqlCategoryRegionalRelation dal = new DAL.MySqlCategoryRegionalRelation();
        ...
        /// 得到分类区域关联对象实体，从缓存中
		/// </summary>
		public Model.CategoryRegionalRelation GetModelByCache(long AutoID)
		{
			...
			return (Model.CategoryRegionalRelation)objModel;
		}
        ...
        /// <param name="desc">选用倒序</param>
        /// <returns>分类区域关联集合</returns>
        public List<Model.CategoryRegionalRelation> GetModelList(int key, string value, int Top, int filedOrder, bool desc)
        {
        ...
        /// 将DateTable转成实体类集合
        /// </summary>
        public List<Model.CategoryRegionalRelation> DataTableToList(DataTable dt)
        {
            List<Model.CategoryRegionalRelation> modelList = new List<Model.CategoryRegionalRelation>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Model.CategoryRegionalRelation model;
